...
export class PIXISkillNodeRenderer implements ISkillNodeRenderer {
    ...
    public CreateTooltip = (node: SkillNode, source: Source) => {
        ...
        if (tooltip === undefined) {
            let title: PIXI.Text | null = node.name.length > 0 ? new PIXI.Text(`${node.name} [${node.GetId()}]`, { fill: 0xFFFFFF, fontSize: 18 }) : null;
    ...
    public CreateConnection = (node: SkillNode, other: SkillNode): PIXI.Sprite | PIXI.Container | null => {
        ...
        if ((node.classStartIndex !== undefined && !node.is(SkillNodeStates.Active)) || (other.classStartIndex !== undefined && !other.is(SkillNodeStates.Active))) {
            return null;
        }
